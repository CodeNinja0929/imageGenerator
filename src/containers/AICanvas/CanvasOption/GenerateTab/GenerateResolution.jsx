...
const Advanced = ({ handle, params }) => {
  useEffect(() => {
    const eleWidth = document.querySelectorAll('input[type=range]')[0];
    const eleHeight = document.querySelectorAll('input[type=range]')[1];

    eleWidth.value = params.generateFrameWidth;
    eleHeight.value = params.generateFrameHeight;

    // handle({['width']: params.generateFrameWidth});
  });
...
const GenerateResolution = ({ handleUpdate, params }) => {
  ...
  return (
    <div className="pt-3 block text-left">
      ...
      {tabToggle == 'simple' ? <Simple handle={handleUpdate} /> : <Advanced handle={handleUpdate} params={params} />}
    </div>
  );
};
